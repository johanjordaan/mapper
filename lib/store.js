...
(function() {
  ...
  define(['../lib/mapper', '../lib/junction'], function(mapper, junction) {
    var exports, get_id, load, load_all, load_collection, remove, save, save_all, _dehidrate, _load, _save;
    ...
    _save = function(store, store_funcs, map, obj, callback) {
      var actions, dehidrated_obj, obj_save_j, ref_list;
      ...
      junction.call(obj_save_j, store_funcs.save, store, map.model_name, dehidrated_obj, function() {});
      junction.call(obj_save_j, store_funcs.save_refs, store, map.model_name, obj, ref_list, function() {});
      junction.call(obj_save_j, store_funcs.add_to_collection, store, map.default_collection, obj, function() {});
    ...
    _load = function(j, store, store_funcs, map, id, callback) {
      var ret_val;
      ret_val = {};
      return store_funcs.load(store, map.model_name, id, function(loaded_object) {
        var actions, ref_field_names;
        if (loaded_object != null) {
          ref_field_names = [];
          mapper.apply(map, ret_val, loaded_object, actions = {
            'List': function(field_name, field_def, obj, source) {
              return ref_field_names.push(field_name);
            }
          });
          return store_funcs.load_refs(store, map.model_name, id, ref_field_names, function(ref_list) {
            ret_val = {
              id: id
            };
            mapper.apply(map, ret_val, loaded_object, actions = {
    ...
    load_collection = function(store, store_funcs, name, callback) {
      return store_funcs.load_collection(store, name, function(ids) {
        return callback(ids);
      });
    };
    load_all = function(store, store_funcs, map, callback) {
      var load_all_j, ret_val;
      load_all_j = junction.create();
      ret_val = [];
      junction.call(load_all_j, load_collection, store, map.default_collection, function(object_ids) {
        var id, _i, _len, _results;
        _results = [];
        for (_i = 0, _len = object_ids.length; _i < _len; _i++) {
          id = object_ids[_i];
          _results.push(junction.call(load_all_j, load, store, store_funcs, id, function(loaded_object) {
            return ret_val.push(loaded_object);
          }));
        }
        return _results;
      });
      return junction.finalise(load_all_j, function(object_ids) {
        return callback(loaded_objects);
      });
    };
    save_all = function(store, store_funcs, map, objects, callback) {
      var obj, save_all_j, _i, _len;
      save_all_j = junction.create();
      for (_i = 0, _len = objects.length; _i < _len; _i++) {
        obj = objects[_i];
        junction.call(save_all_j, save, store, store_funcs, map, obj, function() {
          return obj;
        });
      }
    ...
    save = function(store, store_funcs, map, obj, callback) {
      ...
      for (_i = 0, _len = flat_object_list.length; _i < _len; _i++) {
        ...
        if (obj_map.obj.id === -1) {
          junction.call(id_j, get_id, store, store_funcs, obj_map.map, obj_map.obj, function(id) {
